      ...
      >
        ...
        {question.status === 'RESOLVED' ? (
          <div className="py-6 text-center border border-border-default rounded-lg">
            <p className="text-sm text-text-tertiary">해결된 질문에는 답변을 달 수 없습니다.</p>
          </div>
        ) : isAuthenticated ? (
...
function AnswerItem({
  ...
  useEffect(() => {
    ...
    let cancelled = false
    getMyVote(answer.id)
      .then((res) => { if (!cancelled) setMyVote(res.type) })
      .catch(() => {})
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2 text-sm text-text-tertiary flex-wrap">
          <span className="font-medium text-text-secondary">{answer.author.nickname}</span>
          <span>&middot;</span>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`p-1 rounded transition-colors disabled:cursor-default ${
                myVote === 'UPVOTE' ? 'text-accent-secondary' : 'text-text-tertiary hover:text-accent-secondary'
              }`}
            >
              <svg className="w-3.5 h-3.5" fill={myVote === 'UPVOTE' ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M14 9V5a3 3 0 00-3-3l-4 9v11h11.28a2 2 0 002-1.7l1.38-9a2 2 0 00-2-2.3H14z" />
                <path strokeLinecap="round" strokeLinejoin="round" d="M7 22H4a2 2 0 01-2-2v-7a2 2 0 012-2h3" />
              </svg>
            </button>
            <span className={`text-sm font-medium min-w-[1ch] ${voteCount > 0 ? 'text-accent-secondary' : voteCount < 0 ? 'text-red-400' : 'text-text-secondary'}`}>
              {voteCount}
            </span>
            <button
            ...
            >
              <svg className="w-3.5 h-3.5" fill={myVote === 'DOWNVOTE' ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        ...
        <div className="mt-4 pt-4 border-t border-border-default space-y-3">
          ...
          {isAuthenticated ? (
            ...
            <p className="text-sm text-text-tertiary">
              <Link to="/login" className="text-accent-primary hover:underline">로그인</Link>하면 댓글을 달 수 있어요.
            </p>
          )}
        </div>
      )}
